 ...
 * AbstractInvoker.
 *
 * @see org.apache.dubbo.rpc.proxy.AbstractProxyInvoker
 */
public abstract class AbstractInvoker<T> implements Invoker<T> {
    ...
    /** 从url中取出指定key对应的值, 转为服务调用invoker的附加值. **/
    ...
    /**
     * 做了一些公共的操作，比如服务引用销毁的检测，加入附加值，加入调用链实体域到会话域中等,
     *
    ...
    public Result invoke(Invocation inv) throws RpcException {
        ...
        //当前Invoker设置到Invocation中
        RpcInvocation invocation = (RpcInvocation) inv;
        invocation.setInvoker(this);

        //当前Invoker附加值设置到Invocation中
        if (CollectionUtils.isNotEmptyMap(attachment)) {
            invocation.addObjectAttachmentsIfAbsent(attachment);
        }

        //RpcContext附加值设置到Invocation中
