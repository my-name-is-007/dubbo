 ...
 *
 * storedContext和storedServerContext存储了相关的RpcContext实例的引用，
 * 为的就是防止在回调的时候由于相同的线程用于执行另一个RPC调用导致原来的RpcContext可能已经被更改，
 * 所以存储下来后，我们需要在执行回调之前恢复它
 */
public class AsyncRpcResult implements Result {
     ...
     * has no effect.
     *
     * To avoid this problem, we check the complete status of this future before update it's value.
     *
