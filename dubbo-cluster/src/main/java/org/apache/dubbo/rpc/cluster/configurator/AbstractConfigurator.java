...
public abstract class AbstractConfigurator implements Configurator {
    ...
    public AbstractConfigurator(URL url) {
        if (url == null) { throw new IllegalArgumentException("configurator url == null"); }
        this.configuratorUrl = url;
    }

    @Override
    public URL getUrl() { return configuratorUrl; }
    ...
    private URL configureIfMatch(String host, URL remoteUrl) {
        if (ANYHOST_VALUE.equals(configuratorUrl.getHost()) || host.equals(configuratorUrl.getHost())) {
            // TODO, to support wildcards
            String providers = configuratorUrl.getParameter(OVERRIDE_PROVIDERS_KEY);
            if (StringUtils.isEmpty(providers) || providers.contains(remoteUrl.getAddress()) || providers.contains(ANYHOST_VALUE)) {
                String configApplication = configuratorUrl.getParameter(APPLICATION_KEY,
                        configuratorUrl.getUsername());
                String currentApplication = remoteUrl.getParameter(APPLICATION_KEY, remoteUrl.getUsername());
                    ...
                    for (Map.Entry<String, String> entry : configuratorUrl.getParameters().entrySet()) {
                        ...
                        if (key.startsWith("~") || APPLICATION_KEY.equals(key) || SIDE_KEY.equals(key)) {
                            ...
                            if (value != null && !ANY_VALUE.equals(value)
                                    && !value.equals(remoteUrl.getParameter(key.startsWith("~") ? key.substring(1) : key))) {
                                return remoteUrl;
                            }
                        }
                    }
                    return doConfigure(remoteUrl, configuratorUrl.removeParameters(conditionKeys));
                }
            }
        }
        return remoteUrl;
    ...
    /** 使用后面的URL配置, 覆盖前面的URL配置: 返回新产生的URL配置. **/
